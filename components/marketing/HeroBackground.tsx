...
export function HeroBackground({ x, y }: { x: MotionValue<number>; y: MotionValue<number> }) {
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(rgba(37,99,235,0.055)_1px,transparent_1px),linear-gradient(90deg,rgba(37,99,235,0.055)_1px,transparent_1px)] bg-[size:58px_58px] [mask-image:linear-gradient(to_bottom,black,transparent_88%)]" />
      <motion.div style={{ x: meshX, y: meshY }} className="absolute inset-0">
        <motion.div
          className="absolute -left-40 top-8 h-[30rem] w-[30rem] rounded-full bg-blue-300/25 blur-[120px]"
          animate={reduceMotion ? undefined : { scale: [1, 1.08, 1], x: [0, 28, 0] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute right-[-8rem] top-20 h-[32rem] w-[32rem] rounded-full bg-cyan-300/25 blur-[130px]"
          animate={reduceMotion ? undefined : { scale: [1.05, 0.96, 1.05], y: [0, 24, 0] }}
          transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
        />
        <div className="absolute left-[42%] top-[15%] h-72 w-72 rounded-full bg-sky-200/20 blur-[100px]" />
      </motion.div>
      {particles.map(([left, top, size], index) => (
        <motion.span
          key={`${left}-${top}`}
          className="absolute rounded-full bg-blue-400/45 shadow-[0_0_12px_rgba(56,189,248,0.55)]"
