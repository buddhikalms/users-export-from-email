...
export function HeroBackground({ x, y }: { x: MotionValue<number>; y: MotionValue<number> }) {
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(rgba(0,127,212,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(3,183,178,0.06)_1px,transparent_1px)] bg-[size:58px_58px] [mask-image:linear-gradient(to_bottom,black,transparent_88%)]" />
      ...
      {particles.map(([left, top, size], index) => (
        <motion.span
          key={`${left}-${top}`}
          className="absolute rounded-full bg-brand-turquoise/50 shadow-[0_0_12px_rgba(3,183,178,0.55)]"
