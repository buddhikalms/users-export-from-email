...
export function HeroIllustration() {
  ...
  return (
    <div className="relative mx-auto h-[570px] w-full max-w-[600px] select-none sm:h-[620px]">
      <div className="absolute inset-x-14 bottom-8 h-28 rounded-[50%] bg-blue-500/15 blur-3xl" />
      <svg aria-hidden="true" className="absolute left-1/2 top-8 h-[530px] w-[520px] max-w-none -translate-x-1/2" viewBox="0 0 520 540">
        <defs>
          <linearGradient id="lineGradient" x1="0" x2="1">
            ...
            <stop offset="1" stopColor="#06B6D4" stopOpacity=".72" />
          </linearGradient>
        </defs>
      ...
      >
        <div className="absolute inset-0 translate-y-5 rounded-[1.75rem] bg-blue-600 shadow-[0_28px_55px_-18px_rgba(37,99,235,0.6)] [transform:rotateX(8deg)_rotateY(-8deg)]" />
        <div className="absolute inset-x-0 top-5 h-20 overflow-hidden rounded-[1.75rem] border border-white/30 bg-gradient-to-br from-blue-500 to-cyan-500">
          <div className="absolute inset-x-0 top-0 h-full origin-top bg-blue-700/45 [clip-path:polygon(0_0,50%_63%,100%_0)]" />
      ...
      <div className="absolute left-1/2 top-[155px] z-10 flex -translate-x-1/2 gap-3">
        {["JD", "SB", "AK"].map((initials, index) => (
          ...
          >
            <span className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-blue-100 to-cyan-100 text-[10px] font-bold text-blue-700">{initials}</span>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-blue-50 text-blue-600"><Folder className="h-4 w-4" /></span>
      ...
      <div className="absolute inset-x-0 top-[430px] z-10 flex justify-center gap-3 sm:gap-8">
        {[[FileSpreadsheet, "Excel", "emerald"], [Sheet, "CSV", "blue"], [Cloud, "Marketing", "cyan"]].map(([Icon, label, tone], index) => {
          ...
          return (
            ...
            >
              <span className={`grid h-10 w-10 place-items-center rounded-xl ${tone === "emerald" ? "bg-emerald-50 text-emerald-600" : tone === "cyan" ? "bg-cyan-50 text-cyan-600" : "bg-blue-50 text-blue-600"}`}><ItemIcon className="h-5 w-5" /></span>
